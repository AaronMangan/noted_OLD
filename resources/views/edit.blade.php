        ...
        <div class="mx-auto space-y-6 max-w-7xl sm:px-6 lg:px-8">
            <div class="p-4 bg-white shadow sm:p-8 dark:bg-gray-800 sm:rounded-lg">
                <div class="flex flex-col space-y-1">
                    <form method="post" action="{{ route('pages.update', $page['id']) }}" id="edit_page_form" class="space-y-1">
                        @csrf
                        @method('patch')
                        <div>
                            <x-input-label for="title" class="mb-2 text-lg font-black" :value="__('Title')" />
                            <x-text-input id="title" name="title" type="text" class="block w-full mt-1 text-lg" :value="$page['title'] ?? ''" required autofocus autocomplete="title" />
                        ...
                        <input type="hidden" value="{{$page['content'] ?? ''}}" name="content" id="content">
